...
def create_game():
    ...
    return jsonify(
        {"game_id": game_id, "game_state": game.get_game_state(), "mode": mode}
    )
...
def get_game_state(game_id):
    ...
    return jsonify({**game.get_game_state(), "mode": mode, "step_config": step_config})
...
def get_available_tiles(game_id, score):
    ...
    center_tiles = [
        {"value": tile.value, "worms": tile.worms, "source": "center", "player": None}
    ...
    return jsonify(
        {
            ...
            "best_center_tile": max(center_tiles, key=lambda t: t["value"])
            if center_tiles
            else None,
...
def play_turn(game_id):
    ...
    if "result" in serializable_details and hasattr(
        serializable_details["result"], "value"
    ):
...
def start_auto_play(game_id):
    ...
    def auto_play_worker():
        ...
        while (
            ...
            and step_config["auto_play"]
            and not step_config["paused"]
        ):
            # Jouer un tour
            current_player = game.get_current_player()
            # Sauvegarder le nom du joueur avant de jouer le tour
...
def on_join_game(data):
    ...
    if game_id in games:
        ...
        emit("game_state", {**game_state, "mode": mode, "step_config": step_config})
...
def on_choose_dice_value(data):
    ...
    try:
        ...
        turn_details = {
            "reserved_dice": [
                k.name
                for k in turn_state.reserved_dice.keys()
                for _ in range(turn_state.reserved_dice[k])
            ],
            "remaining_dice_count": turn_state.remaining_dice,
            "current_score": turn_state.get_total_score(),
            "has_worm": turn_state.has_worm(),
            "can_take_tile": turn_state.has_worm()
            and turn_state.get_total_score() >= 21,
            "used_values": [v.name for v in turn_state.used_values],
        }

        # Émettre la mise à jour
        socketio.emit(
            "choice_made", {"success": True, "details": turn_details}, room=game_id
        )
...
def on_choose_tile(data):
    ...
    game_id = data["game_id"]
    tile_choice = data.get(
        "tile_choice"
    )  # {'source': 'center'/'player', 'value': int, 'player': str}
    ...
    if not has_worm:
        emit(
            "error",
            {"message": "Vous devez avoir au moins un ver pour prendre une tuile"},
        )
        return
    ...
    if tile_choice["source"] == "center":
        ...
        if target_player:
            ...
            if (
                top_tile
                and top_tile.value == tile_choice["value"]
                and final_score == tile_choice["value"]
            ):
    ...
    # Émettre le résultat
    emit_turn_played(
        game_id, TurnResult.SUCCESS, turn_details, current_player.name, is_human=True
    )
        ...
        while not game.is_game_over():
            ...
            if not isinstance(current_player.strategy, HumanStrategy):
                ...
                if "result" in serializable_details and hasattr(
                    ...
                    serializable_details["result"] = serializable_details[
                        "result"
                    ].value
...
def on_end_turn(data):
    ...
    if isinstance(current_player.strategy, HumanStrategy):
        ...
        emit_turn_played(
            game_id, result, turn_details, current_player.name, is_human=True
        )
        ...
        def play_ai_turns():
            ...
            while not game.is_game_over():
                ...
                if not isinstance(current_player.strategy, HumanStrategy):
                    ...
                    # Émettre les détails du tour aux clients connectés
                    emit_turn_played(
                        game_id, result, details, player_name, is_human=False
                    )
