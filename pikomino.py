...
class TurnState:
    ...
    def get_total_score(self) -> int:
        ...
        return sum(Dice.get_point_value(value) * count 
    ...
    def can_reserve_value(self, value: DiceValue) -> bool:
        ...
        return (value in self.current_roll and 
                value not in self.used_values)
...
class PikominoGame:
    ...
    def play_turn(self) -> Tuple[TurnResult, Dict]:
        ...
        while turn_state.remaining_dice > 0:
            # Lancer les dés
            turn_state.current_roll = [Dice.roll() for _ in range(turn_state.remaining_dice)]
            ...
            turn_state.reserved_dice[chosen_value] = turn_state.reserved_dice.get(chosen_value, 0) + count
            ...
            if turn_state.remaining_dice > 0 and not current_player.should_continue_turn(turn_state):
                break
        ...
        turn_details['reserved_dice'] = {k.name: v for k, v in turn_state.reserved_dice.items()}
    ...
    def get_game_state(self) -> Dict:
        ...
        return {
            ...
            'tiles_center': [{'value': t.value, 'worms': t.worms} for t in self.tiles_center],
            'player_scores': {p.name: p.get_score() for p in self.players},
            'player_tiles': {p.name: [{'value': t.value, 'worms': t.worms} for t in p.tiles] for p in self.players},
...
def simulate_game(player_names: List[str], strategies: List[GameStrategy] = None) -> Dict:
    """Simule une partie avec les joueurs et stratégies donnés"""
    if strategies is None:
        strategies = [ConservativeStrategy() for _ in player_names]
    
    players = [Player(name, strategy) for name, strategy in zip(player_names, strategies)]
