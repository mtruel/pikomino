...
from pikomino import (
    DiceValue, Dice, Tile, TurnResult, TurnState, 
    Player, ConservativeStrategy, AggressiveStrategy,
    PikominoGame, GameStrategy
)
...
class TestGameSpecialCases:
    ...
    def test_failed_turn_lose_top_tile_and_remove_highest_center(self):
        ...
        assert len(game.tiles_center) == initial_center_tiles - 1  # Une tuile retirée du centre
        assert highest_center_tile not in game.tiles_center  # La plus haute a été retirée
    ...
    def test_steal_tile_exact_score_match(self):
        ...
        turn_state.reserved_dice = {DiceValue.FIVE: 4, DiceValue.WORM: 1, DiceValue.THREE: 1}  # 5*4 + 5*1 + 3*1 = 28
...
class TestTurnFailureScenarios:
    ...
    def test_no_valid_dice_choice_available(self):
        ...
        turn_state = TurnState()
        turn_state.used_values = {DiceValue.ONE, DiceValue.TWO, DiceValue.THREE, 
                                DiceValue.FOUR, DiceValue.FIVE, DiceValue.WORM}
        turn_state.current_roll = [DiceValue.ONE, DiceValue.TWO]  # Valeurs déjà utilisées
    ...
    def test_player_stops_voluntarily(self):
        ...
        turn_state = TurnState()
        turn_state.reserved_dice = {DiceValue.WORM: 1, DiceValue.FIVE: 3}  # 20 points avec ver
...
class TestConservativeStrategy:
    ...
    def test_chooses_most_frequent_when_has_worm(self):
        ...
        turn_state.current_roll = [DiceValue.ONE, DiceValue.ONE, DiceValue.ONE, DiceValue.TWO]
    ...
    def test_stops_when_can_take_tile(self):
        ...
        turn_state = TurnState()
        turn_state.reserved_dice = {DiceValue.WORM: 1, DiceValue.FIVE: 3}  # 5*1 + 5*3 = 20
...
class TestAggressiveStrategy:
    ...
    def test_prioritizes_high_values(self):
        ...
        turn_state = TurnState()
        turn_state.current_roll = [DiceValue.ONE, DiceValue.TWO, DiceValue.FIVE, DiceValue.WORM]
        turn_state.used_values = set()
...
class TestGameIntegration:
    ...
    def test_successful_turn(self, mock_roll):
        ...
        mock_roll.side_effect = [
            ...
            DiceValue.FIVE, DiceValue.FIVE, DiceValue.FIVE, DiceValue.FIVE,
            DiceValue.ONE, DiceValue.TWO, DiceValue.THREE, DiceValue.FOUR,
        ] + [
            # Deuxième lancer : des vers parmi les 4 restants
            DiceValue.WORM, DiceValue.WORM, DiceValue.ONE, DiceValue.TWO
        ]
        
        # Stratégie personnalisée pour le test
        def side_effect(turn_state, player):
            if DiceValue.FIVE in turn_state.current_roll and DiceValue.FIVE not in turn_state.used_values:
                return DiceValue.FIVE
            elif DiceValue.WORM in turn_state.current_roll and DiceValue.WORM not in turn_state.used_values:
                return DiceValue.WORM
            return None
        
        players[0].strategy = MockStrategy()
        players[0].strategy.choose_dice_value = side_effect
        players[0].strategy.should_continue_turn = lambda ts, p: ts.remaining_dice > 0 and not ts.has_worm()
    ...
    def test_failed_turn_no_worm(self, mock_roll):
        ...
        players[0].strategy.should_continue_turn = lambda ts, p: False  # S'arrête immédiatement
        ...
        assert len(game.tiles_center) == initial_center_count - 1  # Tuile retirée du centre
    ...
    def test_failed_turn_insufficient_score(self, mock_roll):
        ...
        mock_roll.side_effect = [
            ...
            DiceValue.WORM, DiceValue.ONE, DiceValue.ONE, DiceValue.ONE,
            DiceValue.ONE, DiceValue.ONE, DiceValue.ONE, DiceValue.ONE
        ]
        ...
        players[0].strategy.should_continue_turn = lambda ts, p: False  # S'arrête immédiatement après le premier lancer
...
class TestRuleComplianceEdgeCases:
    ...
    def test_must_stop_when_no_dice_remain(self):
        ...
        turn_state = TurnState()
        turn_state.remaining_dice = 0
        turn_state.reserved_dice = {DiceValue.WORM: 1, DiceValue.FIVE: 7}  # Tous les dés utilisés
...
class TestNewDiceLogic:
    ...
    def test_must_choose_dice_value_before_reroll(self):
        ...
        available_choices = [v for v in turn_state.current_roll if v not in turn_state.used_values]
    ...
    def test_forced_turn_end_when_no_valid_choices(self):
        ...
        has_valid_choices = any(turn_state.can_reserve_value(v) for v in turn_state.current_roll)
...
class TestBugFixes:
    ...
    def test_strict_dice_choice_enforcement(self):
        ...
        turn_state.current_roll = [DiceValue.ONE, DiceValue.ONE]  # Que des valeurs déjà utilisées
    ...
    def test_forced_turn_end_no_valid_choices(self):
        ...
        turn_state.used_values = {DiceValue.ONE, DiceValue.TWO}  # Toutes les valeurs du lancer déjà utilisées
        turn_state.remaining_dice = 5
        
        # Vérifier qu'aucun choix valide n'est disponible
        available_choices = [v for v in turn_state.current_roll if turn_state.can_reserve_value(v)]
...
class TestIntegrationWithNewFeatures:
    ...
    def test_complete_turn_with_tile_theft(self, mock_roll):
        ...
        mock_roll.side_effect = [
            ...
            DiceValue.WORM, DiceValue.WORM, DiceValue.WORM, DiceValue.WORM, DiceValue.WORM,  # 25 points
            DiceValue.ONE, DiceValue.ONE, DiceValue.ONE
        ]
...
class TestWebInterface:
    ...
    def test_create_game_api(self):
        ...
        with app.test_client() as client:
            response = client.post('/api/create_game', json={
                'players': ['Alice', 'Bob'],
                'mode': 'interactive'
            })
