...
def run_simulation(num_games=10):
    ...
    for game_num in range(num_games):
        ...
        players = [
            ...
            for name, strategy in zip(player_names, game_strategies)
        ]
        game = PikominoGame(players)
    ...
    for player_name, player_stats in stats.items():
        ...
        success_rate = (
            player_stats["successful_turns"]
            / (player_stats["successful_turns"] + player_stats["failed_turns"])
        ) * 100
    ...
    create_bar_plot(
        ax1, win_rates, "Taux de victoire", "Pourcentage (%)", percentage=True
    )
    ...
    success_rates = [
        (
            stats[p]["successful_turns"]
            / (stats[p]["successful_turns"] + stats[p]["failed_turns"])
        )
        * 100
        for p in players
    ]
    ...
    create_bar_plot(
        ax4, avg_tiles, "Nombre moyen de tuiles conservées", "Nombre de tuiles"
    )
    ...
    legend_elements = [
        plt.Rectangle((0, 0), 1, 1, facecolor=color, label=strat)
